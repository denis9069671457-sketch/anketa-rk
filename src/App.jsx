    ...
    <div style={{ maxWidth:820, margin:"0 auto", padding:"28px 20px" }}>
      <div style={{ background:"linear-gradient(135deg,#1a2a2a,#2a1a1a)", borderRadius:16, padding:"24px 28px", marginBottom:20 }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.08)", borderRadius:10, padding:"14px 18px", fontSize:13, color:"rgba(255,255,255,0.8)", lineHeight:1.7 }}>
          Уважаемые родители! Все материалы необходимо прислать <b style={{color:C.yellow}}>не позднее чем за 3 суток</b> до начала диагностики. Прикрепите файл — галочка поставится автоматически.
        </div>
      </div>

      {prevDocs && (
        <div style={{ background:"#fff8e1", borderLeft:"4px solid #f5c842", borderRadius:"0 12px 12px 0", padding:"14px 18px", marginBottom:16, fontSize:13, color:"#555", lineHeight:1.7 }}>
          Найдена предыдущая отправка от {new Date(prevDocs.date).toLocaleDateString("ru-RU")}. Добавьте недостающие документы.
        </div>
      )}
      ...
      {DOCUMENTS.map(docGroup => (
        <div key={docGroup.id} style={{ background:C.white, borderRadius:14, boxShadow:"0 2px 12px rgba(0,0,0,0.06)", marginBottom:16, overflow:"hidden" }}>
          <div style={{ padding:"14px 20px", borderBottom: docGroup.required ? "2px solid #fee2e2" : "2px solid #e8f8f8", display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:15, fontWeight:700, color:C.dark }}>{docGroup.category}</span>
          ...
          <div style={{ padding:"12px 20px" }}>
            {docGroup.items.map(item => {
              ...
              const isChecked = checked[item.id];
              return (
                <div key={item.id} style={{ marginBottom:16, paddingBottom:16, borderBottom:"1px solid #e2e8f0" }}>
                  <div style={{ display:"flex", alignItems:"flex-start", gap:12, marginBottom:10, cursor: hasFile ? "pointer" : "not-allowed", opacity: hasFile ? 1 : 0.7 }}
                    onClick={() => { if (hasFile) toggleCheck(item.id); }}>
                    <div style={{ width:22, height:22, borderRadius:5,
                      border: "2px solid " + (isChecked ? C.teal : hasFile ? "#aaa" : "#ddd"),
                      background: isChecked ? C.teal : "#fff",
                    ...
                    <div>
                      <p style={{ margin:0, fontSize:14, fontWeight:600, color: isChecked ? C.tealDark : C.dark }}>{item.label}</p>
                      {item.note && <p style={{ margin:"2px 0 0", fontSize:12, color:C.grayMid }}>{item.note}</p>}
                      {!hasFile && <p style={{ margin:"3px 0 0", fontSize:11, color:"#e84545" }}>⚠️ Сначала прикрепите файл</p>}
                    </div>
                  </div>
                  <div style={{ marginLeft:34 }}>
                    {itemFiles.map((f, fi) => (
                      <div key={fi} style={{ display:"flex", alignItems:"center", gap:8, background:C.tealLight, borderRadius:8, padding:"7px 12px", marginBottom:6 }}>
                        ...
                        <button onClick={() => removeFile(item.id, fi)} style={{ background:"none", border:"none", color:"#e84545", cursor:"pointer", fontSize:18, padding:0 }}>×</button>
                      </div>
                    ))}
                    <label style={{ display:"inline-flex", alignItems:"center", gap:6, cursor:"pointer", padding:"6px 14px", background:C.grayLight, borderRadius:8, border:"1px dashed #ccc", fontSize:12, color:C.grayMid }}>
                      <span>📎</span> {hasFile ? "Добавить ещё файл" : "Прикрепить файл или фото"}
                      <input type="file" accept=".jpg,.jpeg,.png,.heic,.pdf,.doc,.docx" multiple style={{ display:"none" }} onChange={e => handleFile(item.id, e)} />
                    </label>
                  </div>
                </div>
              );
      ...
      <div style={{ background:C.white, borderRadius:14, boxShadow:"0 2px 12px rgba(0,0,0,0.06)", padding:"20px", marginBottom:20 }}>
        ...
        <textarea rows={3} value={comment} onChange={e => setComment(e.target.value)}
          placeholder="Например: анализ ЭЭГ сдаём на следующей неделе..."
          style={{ width:"100%", border:"1.5px solid #e2e8f0", borderRadius:8, padding:"10px 14px", fontSize:14, color:C.dark, outline:"none", resize:"vertical", boxSizing:"border-box", background:"#fafcfc", fontFamily:"inherit" }}
        />
      </div>
